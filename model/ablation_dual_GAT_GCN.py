...
class GNN_branch_with_GCN(torch.nn.Module):
    def __init__(self):
        super(GNN_branch_with_GCN,self).__init__()
        self.graphconv=Sequential('x,edge_index,batch',[
            (GCNConv(9, 128), 'x,edge_index -> x'),
            nn.LeakyReLU(),
            GraphNorm(128),
            (nn.Dropout(p=0.1), 'x -> x'),
            ...
            GraphNorm(512),
            (nn.Dropout(p=0.1), 'x -> x'),
            (GCNConv(512, 767 * 2), 'x,edge_index -> x'),
            nn.LeakyReLU(),
            GraphNorm(767 * 2),
...
class Net(torch.nn.Module):
    def __init__(self, n_output_layers=1):
        ...
        self.gnn_GAT_branch=GNN_branch_with_GAT()
        self.gnn_GCN_branch=GNN_branch_with_GCN()
    ...
    def forward(self,data):
        GAT_memory = self.gnn_GAT_branch(data)
        GCN_memory = self.gnn_GCN_branch(data)
        GAT_memory = GAT_memory.view(-1,4,767)
        GCN_memory = GCN_memory.view(-1,4,767)
        ...
        final_representation = self.decodelayer1(GAT_memory, GCN_memory)
