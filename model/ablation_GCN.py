...
class Net(torch.nn.Module):
    def __init__(self, n_output_layers=1):
        ...
        self.gnn_branch=GNN_branch_with_GCN()
    ...
    def forward(self, data):
        ...
        memory = memory.view(memory.shape[0], -1)

        # outputs = self.model(**inputs)
        # outputs = outputs[0]
        # outputs = outputs[:, -1, :].view(outputs.shape[0], -1)

        # cat_output = torch.cat((memory, outputs), dim=1)

        x = self.outputlayer(memory)
        return x
