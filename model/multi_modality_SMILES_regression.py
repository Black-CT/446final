...
class Net(torch.nn.Module):
    def __init__(self, n_output_layers=1):
        ...
        # )
        self.fc767_512 = nn.Sequential(
            nn.Linear(767, 512),
            nn.LeakyReLU(),
            nn.BatchNorm1d(512),
        )

        self.fc512_512 = nn.Sequential(
            nn.Linear(512, 512),
            nn.LeakyReLU(),
            nn.BatchNorm1d(512),
        )

        self.fc512_256 = nn.Sequential(
            nn.Linear(512, 256),
            nn.LeakyReLU(),
            nn.BatchNorm1d(256),
        )

        self.fc256_256 = nn.Sequential(
            nn.Linear(256, 256),
            nn.LeakyReLU(),
            nn.BatchNorm1d(256),
        )

        self.fc256_20_1 = nn.Sequential(
            nn.Linear(256, 128),
            nn.LeakyReLU(),
            nn.BatchNorm1d(128),
            nn.Dropout(0.5),
        ...
        # self.fc256_128 = nn.Sequential(
        #     nn.Linear(256, 128),
        #     nn.LeakyReLU(),
        #     nn.BatchNorm1d(128),
        # )
        #
        # self.fc128_128 = nn.Sequential(
        #     nn.Linear(128, 128),
        #     nn.LeakyReLU(),
        #     nn.BatchNorm1d(128),
        # )
        #
        # self.fc128_1 = nn.Sequential(
        #     nn.Linear(128, 1),
        #     nn.Dropout(0.5),
        # )
    ...
    def forward(self, data, inputs):
        ...
        x = final_representationfinal_representation

        # resdiual
        first = self.fc767_512(x)
        second = self.fc512_512(first) + first

        third = self.fc512_256(second)
        fourth = self.fc256_256(third) + third

        output = self.fc256_20_1(fourth)

        # fifth = self.fc256_128(fourth)
        # sixth = self.fc128_128(fifth) + fifth
        #
        # output = self.fc128_1(sixth)
